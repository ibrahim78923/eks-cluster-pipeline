...
import { getgradewiserequest } from 'src/services/request.service'
...
const GradeWise = () => {
  ...
  const handleSend = async (handleSend, dateFrom, dateTo) => {
    setTableLoading(true)
    const response = await getgradewiserequest(handleSend, dateFrom, dateTo)
    if (response?.success === true) {
      setTableLoading(false)
      setGradeWise(response.requests)
            ...
            >
              <Box>
                <Card style={{marginBottom: 10, padding: 10}}>
                  <FilterForm setGradeWise={setGradeWise} schools={schools} handleSend={handleSend} settings={settings} />
                </Card>
                <Card style={{ minHeight: 750, padding: 10 }}>
                  ...
                  <Table
                    ...
                    dataSource={gradewise || []}
                    pagination={
                      gradewise?.length > 10
                        ? {
                            defaultCurrent: 1,
                            total: gradewise?.length,
                            defaultPageSize: 10,
