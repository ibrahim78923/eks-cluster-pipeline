...
import { RequestsBody } from 'src/views/teacherRequests/confirmedRequests'
import { SocketProvider, useSocket } from 'src/context/socket/teacherSocket'

const ConfirmedRequests = props => {
  return (
    <RequestProvider>
      <SocketProvider>
        <Main {...props} />
    ...
    if (socket) {
      ...
      socket.on('_teacher_requests', data => {
        ...
        const newRequestData = data
          ?.filter(item => item.createdAt.slice(0, 10) === today && item.status === 2)
