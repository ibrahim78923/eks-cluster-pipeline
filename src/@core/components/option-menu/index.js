...
const OptionsMenu = props => {
  // ** Props
  const { icon, options, menuProps, iconProps, leftAlignMenu, iconButtonProps, setValue, value } = props
      ...
      >
        {options.map((option, index) => {
          if (typeof option === 'string') {
            return (
              <MenuItem selected={value === option} value={option} key={index} onClick={()=>handleClickMenu(option)}>
                {option}
              </MenuItem>
            )
