...
const HorizontalLayout = props => {
  ...
  const [isLoginFromSuperAdmin,setLoginFromSuperAdmin] = useState(false);
  const [isLoading,setLoading] = useState(false);

  const auth = useAuth()

  useEffect(()=> {
    const data = JSON.parse(window.localStorage.getItem('isFromSuperAdmin'))
    console.log("isLoginFromSuperAdmin", data);
    setLoginFromSuperAdmin(data?.isFromSuperAdmin)
  },[router])

  const handleAdminLogin = ()=> {
    setLoading(true);
    auth.login(
      { email: process.env.NEXT_PUBLIC_ADMIN_EMAIL, password: process.env.NEXT_PUBLIC_ADMIN_PASSWORD, role: null },
      ({ success, message }) => {
        if (success) {
    ...
    <HorizontalLayoutWrapper className='layout-wrapper'>
      <MainContentWrapper className='layout-content-wrapper' sx={{ ...(contentHeightFixed && { maxHeight: '100vh' }) }}>
        ...
        >
          ...
          >
            {!router.pathname.includes('requests' || 'newRequests') && (
              <Toolbar
                ...
                >
                  ...
                  {isLoginFromSuperAdmin && (
                    <Button
                      ...
                      sx={{ml: 2}}
                    >
                     {isLoading ? <CircularProgress size={24} color="inherit" /> : 'Back To Super Admin'}
                    </Button>
                  )}
                </Toolbar>
              )}
        ...
        {!router.route.includes('/requests') && (
