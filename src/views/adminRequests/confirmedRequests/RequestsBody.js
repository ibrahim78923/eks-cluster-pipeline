...
const RequestsBody = ({ grades }) => {
  ...
  useEffect(() => {
    const sortedSyudents = gradeIds?.includes('all')
      ? requests?.filter(item => item.status === 2 && auth?.user?.id === item?.clientId)
      : requests?.filter(
          item => gradeIds?.includes(item.gradeId) && item.status === 2 && auth?.user?.id === item?.clientId
        )
        ...
        {sortedReq?.length > 0 ? (
          ...
          <Box>
            <Typography variant='h5' sx={{ textAlign: 'center', mt: 30 }}>
              No confirmed requests found yet
            </Typography>
          </Box>
        )}
      </Box>
