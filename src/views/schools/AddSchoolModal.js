...
import { DialogTitle, Stack, Tooltip, colors, Alert } from '@mui/material'
...
const AddSchoolModal = ({ isOpen, handleClose, onSubmit, isLoaded, placesData }) => {
  const auth = useAuth()
  ...
  const handleSubmit = async (e) => {
    ...
    if (result?.success) {
      toast.success('School has been added')
      setLoading(false);
    ...
    setLoading(false);
    // setErrors("Please Select Location")
  }}
  else{
            ...
            <Grid item xs={12} sm={5}>
              {placeholderImg === '' && (
                ...
                >
                  ...
                  {selectedImage && (
                    <Box display='flex' flexDirection='column' justifyContent='baseline'>
                      <div>
                        <img alt='not found' width={'80px'} src={URL.createObjectURL(selectedImage)} />
                      </div>
                      <div>
