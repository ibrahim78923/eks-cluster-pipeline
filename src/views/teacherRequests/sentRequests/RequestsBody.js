...
const SentRequestsBody = ({ grades }) => {
  ...
  useEffect(() => {
    const sortedSyudents = gradeIds?.includes('all')
      ? requests?.filter(item => item.status === 1 && auth?.user?.clientId === item?.clientId)
      : requests?.filter(
          item => gradeIds?.includes(item.gradeId) && item.status === 1 && auth?.user?.clientId === item?.clientId
        )
