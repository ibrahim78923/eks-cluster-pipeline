...
const RequestsBody = ({ grades }) => {
  ...
  useEffect(() => {
    const sortedSyudents = requests
      ?.filter(item => item.status === 0 && auth?.user?.clientId === item?.clientId)
      ?.sort((a, b) => {
        ...
        <Box sx={{ display: 'flex', width: '100%', flexWrap: 'wrap', gap: 10 }}>
          <Box>
            <Select
              ...
              id='my-select'
              multiple
              value={gradeIds?.length === grades?.length ? ['all'] : gradeIds}
            ...
            >
              <MenuItem sx={{ height: 33, padding: 0, margin: 0, ml: 3 }} key={'all'} value={'all'}>
                ...
                <Checkbox checked={gradeIds?.length === grades?.length} />
              </MenuItem>
              {grades.map(item => (
                <MenuItem sx={{ height: 33, padding: 0, margin: 0, ml: 3 }} key={item.id} value={item.id}>
                  <ListItemText primary={item.name} />
                  <Checkbox checked={gradeIds?.length !== grades?.length && gradeIds?.indexOf(item.id) > -1} />
                </MenuItem>
              ))}
            </Select>
          </Box>
...
const GradeCol = ({ grades, gradeIds, nameOnly, isMobile, isAllGrades, grade }) => {
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
        ...
        <Tooltip title={student?.name ? student.name : student?.nameAr}>
          <Box
            ...
            sx={{ maxHeight: 40, color: '#000000', ml: 1, fontSize: nameOnly ? '15px' : '12px', width: '120px' }}
          >
